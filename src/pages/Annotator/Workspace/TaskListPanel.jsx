...
// Panel danh sách task bên trái workspace: hiển thị toàn bộ item, trạng thái và cho phép chuyển task
const TaskListPanel = ({ tasks, currentTaskId, onSelect, projectName }) => {
  // Đếm số lượng task theo từng trạng thái để hiển thị badge tổng quan ở header
  ...
  // Lấy cấu hình màu/label cho badge trạng thái, fallback về 'assigned' nếu không tìm thấy
  const getStatusConfig = (status) => TASK_STATUS[status] || TASK_STATUS.assigned;

  // Sắp xếp tasks theo độ ưu tiên: rejected > revised > in_progress > assigned > completed > submitted > approved
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900 border-r border-gray-700">
      <div className="p-4 border-b border-gray-700 shrink-0">
        <h3 className="text-sm font-bold text-gray-200 mb-1">Danh sách Item</h3>
      ...
      <div className="flex-1 overflow-y-auto">
        {sortedTasks.map((task, idx) => {
          ...
          return (
            ...
            >
              {/* Task đã hoàn thành hiển thị icon check, còn lại hiển thị chấm màu trạng thái */}
              {task.status === 'completed' ? (
                ...
                // Task bị trả lại (rejected) nhấp nháy để thu hút sự chú ý
        ...
        {sortedTasks.length === 0 && (
          <div className="p-6 text-center text-gray-500 text-sm">Không có item nào trong project này</div>
        )}
      </div>
    </div>
  );
