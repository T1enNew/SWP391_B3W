...
import {
  ...
  IconButton, InputAdornment, Snackbar, Stack,
...
const STATUS_CFG = {
  active:    { label: 'Active',     color: '#22c55e', bg: 'rgba(34,197,94,0.12)',   border: 'rgba(34,197,94,0.3)',   icon: <ActiveIcon  sx={{ fontSize: 13 }} /> },
  draft:     { label: 'Draft',      color: '#94a3b8', bg: 'rgba(148,163,184,0.1)',  border: 'rgba(148,163,184,0.2)', icon: <DraftIcon   sx={{ fontSize: 13 }} /> },
  completed: { label: 'Completed',  color: '#3b82f6', bg: 'rgba(59,130,246,0.12)',  border: 'rgba(59,130,246,0.3)',  icon: <CheckIcon   sx={{ fontSize: 13 }} /> },
  archived:  { label: 'Archived',   color: '#f59e0b', bg: 'rgba(245,158,11,0.12)',  border: 'rgba(245,158,11,0.3)',  icon: <ArchiveIcon sx={{ fontSize: 13 }} /> },
};
...
const getDeadlineState = (deadline) => {
  ...
  if (diff < 0)        return { label: 'Quá hạn',   color: '#ef4444', bg: 'rgba(239,68,68,0.1)'   };
  if (hours < 24)      return { label: 'Hôm nay',   color: '#f97316', bg: 'rgba(249,115,22,0.1)'  };
  if (hours < 72)      return { label: 'Sắp đến',   color: '#f59e0b', bg: 'rgba(245,158,11,0.1)'  };
  return null;
};
...
const StatTile = ({ icon, value, label, accent, active, onClick }) => (
  <Box onClick={onClick} sx={{
    ...
    borderRadius: 3, px: 2.5, py: 2, cursor: 'pointer', transition: 'all 0.15s',
    '&:hover': { borderColor: 'rgba(59,130,246,0.3)', bgcolor: 'rgba(59,130,246,0.05)' },
  }}>
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
      <Box sx={{ color: active ? PRIMARY : MUTED }}>{icon}</Box>
      <Typography sx={{ color: MUTED, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.6 }}>{label}</Typography>
    </Box>
    <Typography sx={{ color: active ? PRIMARY : TEXT, fontSize: 26, fontWeight: 800, lineHeight: 1 }}>{value}</Typography>
  </Box>
);

// ── Info Dialog Row ────────────────────────────────────────────────────────────
const InfoRow = ({ label, value, color }) => (
  <Box sx={{ display: 'flex', gap: 1.5, py: 1, borderBottom: `1px solid ${BORDER}` }}>
    <Typography sx={{ color: MUTED, fontSize: 13, minWidth: 130, flexShrink: 0 }}>{label}</Typography>
    <Typography sx={{ color: color || TEXT, fontSize: 13, fontWeight: 600, wordBreak: 'break-word' }}>{value ?? 'N/A'}</Typography>
  </Box>
);

// ── Project Card ───────────────────────────────────────────────────────────────
const ProjectCard = ({ project, datasets, onInfo, onDelete, onNavigate }) => {
  const cfg    = getCfg(project.status);
  const dlState = getDeadlineState(project.deadline);
  const dsName  = project.dataset_name || datasets.find(d => d.id === project.dataset_id)?.name || 'N/A';
    ...
    <Box sx={{
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
          ...
          <Box sx={{
            ...
            borderRadius: 10, px: 1.2, py: 0.35,
          }}>
          ...
          <Tooltip title="Thông tin đầy đủ">
            <IconButton size="small" onClick={() => onInfo(project)} sx={{
              color: MUTED, width: 26, height: 26,
      ...
      <Stack spacing={1.2} sx={{ flex: 1 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Typography sx={{ color: TEXT, fontSize: 12.5, fontWeight: 600, maxWidth: 160, textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={dsName}>{dsName}</Typography>
        </Box>

        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography sx={{ color: MUTED, fontSize: 12.5 }}>Tasks</Typography>
          <Typography sx={{ color: TEXT, fontSize: 12.5, fontWeight: 600 }}>{project.total_tasks} tác vụ</Typography>
        </Box>
      ...
      <Box sx={{ display: 'flex', gap: 1, pt: 1.5, borderTop: `1px solid ${BORDER}` }}>
        <Button
          ...
          sx={{
            bgcolor: PRIMARY, borderRadius: 2, textTransform: 'none', fontWeight: 700, fontSize: 13,
            boxShadow: 'none', '&:hover': { bgcolor: '#2563eb' },
          }}>
          Chi tiết
        </Button>
        <Tooltip title="Xóa project">
          <IconButton size="small" onClick={() => onDelete(project)} sx={{
...
export default function Projects() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (datasetRes.status === 'fulfilled') setDatasets(getArray(datasetRes.value.data));
    } catch (e) {
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: BG }}>
      ...
      <Box sx={{ px: 4, pt: 4, pb: 3, borderBottom: `1px solid ${BORDER}`, bgcolor: PANEL }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 2, mb: 3 }}>
          <Box>
            ...
            <Typography sx={{ color: MUTED, fontSize: 14, ml: 0.5 }}>Quản lý toàn bộ dự án annotation</Typography>
          </Box>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/manager/projects/create')}
              sx={{
        ...
        <Stack direction="row" spacing={2} flexWrap="wrap" useFlexGap>
          <StatTile icon={<TaskIcon sx={{ fontSize: 16 }} />}     value={counts.all}       label="Tất cả"     accent="#6366f1"  active={statusFilter === 'all'}       onClick={() => setStatusFilter('all')} />
          <StatTile icon={<ActiveIcon sx={{ fontSize: 16 }} />}   value={counts.active}    label="Active"     accent="#22c55e"  active={statusFilter === 'active'}     onClick={() => setStatusFilter('active')} />
          <StatTile icon={<CheckIcon sx={{ fontSize: 16 }} />}    value={counts.completed} label="Completed"  accent="#3b82f6"  active={statusFilter === 'completed'}  onClick={() => setStatusFilter('completed')} />
          <StatTile icon={<DraftIcon sx={{ fontSize: 16 }} />}    value={counts.draft}     label="Draft"      accent="#94a3b8"  active={statusFilter === 'draft'}      onClick={() => setStatusFilter('draft')} />
          {counts.archived > 0 && (
            <StatTile icon={<ArchiveIcon sx={{ fontSize: 16 }} />} value={counts.archived} label="Archived"   accent="#f59e0b"  active={statusFilter === 'archived'}   onClick={() => setStatusFilter('archived')} />
          )}
        </Stack>
      </Box>
      ...
      <Box sx={{ p: 4 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
          <TextField
            size="small" placeholder="Tìm theo tên project, mô tả, dataset..."
        ...
        {loading && projects.length === 0 ? (
          ...
          <Box sx={{
            ...
            <Box sx={{ width: 72, height: 72, borderRadius: '50%', bgcolor: 'rgba(59,130,246,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Typography sx={{ color: MUTED, fontSize: 13 }}>
              {search ? `Kết quả cho "${search}"` : 'Tạo project đầu tiên để bắt đầu annotation'}
            </Typography>
            {!search && (
              <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/manager/projects/create')}
                sx={{ background: 'linear-gradient(135deg,#3b82f6,#2563eb)', textTransform: 'none', fontWeight: 700, borderRadius: 2, px: 4 }}>
                Tạo project mới
              </Button>
            )}
          </Box>
      ...
      <Dialog open={deleteDialog.open} onClose={() => setDeleteDialog({ open: false, project: null })}
        ...
        <DialogContent sx={{ pt: 2.5 }}>
          ...
          <Typography sx={{ color: '#ef4444', fontSize: 13, mt: 1 }}>⚠ Hành động này không thể hoàn tác.</Typography>
        </DialogContent>
        <DialogActions sx={{ borderTop: `1px solid ${BORDER}`, px: 3, py: 2, gap: 1 }}>
          <Button onClick={() => setDeleteDialog({ open: false, project: null })} sx={{ color: MUTED, textTransform: 'none' }}>Hủy</Button>
      ...
      <Dialog open={!!infoProject} onClose={() => setInfoProject(null)} maxWidth="sm" fullWidth
        PaperProps={{ sx: { bgcolor: PANEL, border: `1px solid ${BORDER}`, borderRadius: 3, color: TEXT } }}>
        <DialogTitle sx={{ fontWeight: 800, borderBottom: `1px solid ${BORDER}`, pb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box>
            <Typography sx={{ fontWeight: 800, fontSize: 17 }}>{infoProject?.name}</Typography>
            {infoProject && (
              <Box sx={{
                display: 'inline-flex', alignItems: 'center', gap: 0.6, mt: 0.8,
                bgcolor: getCfg(infoProject.status).bg, border: `1px solid ${getCfg(infoProject.status).border}`,
                borderRadius: 10, px: 1.2, py: 0.3,
              }}>
                <Box sx={{ color: getCfg(infoProject.status).color }}>{getCfg(infoProject.status).icon}</Box>
                <Typography sx={{ color: getCfg(infoProject.status).color, fontSize: 11, fontWeight: 700 }}>
                  {getCfg(infoProject.status).label}
                </Typography>
              </Box>
            )}
          </Box>
          <IconButton onClick={() => setInfoProject(null)} size="small" sx={{ color: MUTED, '&:hover': { color: TEXT } }}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          ...
          {infoProject?.updatedAt && <InfoRow label="Cập nhật cuối" value={fmtDateTime(infoProject?.updatedAt)} />}
          {infoProject?.guidelines && <InfoRow label="Guidelines" value={infoProject?.guidelines} />}
          {infoProject?.reviewer && (
            <InfoRow label="Reviewer" value={infoProject.reviewer?.fullName || infoProject.reviewer?.username || infoProject.reviewer} color="#a78bfa" />
          )}
        ...
        <DialogActions sx={{ borderTop: `1px solid ${BORDER}`, px: 3, py: 2, gap: 1 }}>
          <Button onClick={() => setInfoProject(null)} sx={{ color: MUTED, textTransform: 'none' }}>Đóng</Button>
          <Button onClick={() => navigate(`/manager/projects/${infoProject?.id}`)} variant="contained"
            startIcon={<VisibilityIcon />}
            sx={{ background: 'linear-gradient(135deg,#3b82f6,#2563eb)', textTransform: 'none', fontWeight: 700, borderRadius: 2, px: 3 }}>
            Xem chi tiết
          </Button>
        </DialogActions>
      </Dialog>
