...
const getFullImageUrl = (dataItem) => {
  ...
  const rawPath = (dataItem?.path || directUrl || "")
    .replace(/\\/g, "/")
    .replace(/^\/+/, "");
  if (rawPath) {
    const uploadsIdx = rawPath.indexOf("uploads/");
    const relativePath =
      uploadsIdx !== -1 ? rawPath.substring(uploadsIdx) : rawPath;
...
const TypeBadge = ({ type }) => {
  const t = {
    image: { bg: "rgba(245,158,11,0.15)", color: "#f59e0b" },
    audio: { bg: "rgba(244,114,182,0.15)", color: "#f472b6" },
    text: { bg: "rgba(52,211,153,0.15)", color: "#34d399" },
  }[type] || { bg: "rgba(245,158,11,0.15)", color: "#f59e0b" };
  return (
    <Chip
...
const Datasets = () => {
  ...
  const getDsStatus = (stat) => {
    if (!stat)
      return { status: "not_started", label: "Chua bat dau", color: "#64748b" };
    ...
    if (totalRawItems === 0 || approved === 0) {
      return pending > 0 || submitted > 0
        ? { status: "annotating", label: "Dang ghi nhan", color: "#3b82f6" }
        : { status: "not_started", label: "Chua bat dau", color: "#64748b" };
    }
    const pct = Math.min((approved / totalRawItems) * 100, 100);
    if (pct >= 100)
      return { status: "ready", label: "San sang AI", color: "#22c55e" };
    if (submitted > 0)
      return { status: "under_review", label: "Dang review", color: "#f59e0b" };
  ...
  const fetchDatasets = async () => {
    try {
      ...
      const normalized = all.map(ds => ({
        ...ds,
        createdAt: ds.createdAt || ds.created_at,
        totalItems: ds.totalItems || ds.total_items || ds.files?.length || 0,
      }));
      setDatasets(normalized);

      // Gọi status từng dataset một để tránh overload backend
      const entries = [];
      for (const ds of all) {
        if (!ds?.id) continue;
        try {
          ...
          const dsData = s.data || {};
          entries.push([ds.id, dsData]);

          // Find subtopic ID from all possible fields
          const subtopicId =
            dsData.subtopicId ||
            dsData.subtopic_id ||
            (Array.isArray(dsData.subtopicIds) ? dsData.subtopicIds[0] : null) ||
            (Array.isArray(dsData.subtopics) ? dsData.subtopics[0]?.id : null) ||
            null;

          // Collect ALL subtopic IDs to fetch assets from
          const allStIds = [];
          if (Array.isArray(dsData.subtopicIds)) {
            allStIds.push(...dsData.subtopicIds.filter(Boolean));
          }
          if (Array.isArray(dsData.subtopics)) {
            dsData.subtopics.forEach(s => {
              const id = s.id || s.subtopicId;
              if (id && !allStIds.includes(id)) allStIds.push(id);
            });
          }
          if ((dsData.subtopicId || dsData.subtopic_id) && !allStIds.includes(dsData.subtopicId || dsData.subtopic_id)) {
            allStIds.push(dsData.subtopicId || dsData.subtopic_id);
          }

          // If no subtopics found, fetch them from topic
          if (allStIds.length === 0 && dsData.topic_id) {
            try {
              const topicRes = await axios.get(`${API_URL}/api/subtopics?topic_id=${dsData.topic_id}`);
              const topicSubtopics = getArray(topicRes.data);
          ...
          // Count items from ALL subtopics
          let itemCount = 0;
          for (const stId of allStIds) {
            try {
              const assetRes = await axios.get(`${API_URL}/api/subtopics/${stId}/assets`);
              if (Array.isArray(assetRes.data)) {
                itemCount += assetRes.data.length;
              }
            } catch {}
          }
          ...
          if (itemCount > 0) {
            setDatasets(prev => prev.map(d =>
              d.id === ds.id ? { ...d, totalItems: itemCount } : d
            ));
          }
        } catch {
          entries.push([ds.id, null]);
        }
      }
    ...
    if (highlightedDsId && datasets.length > 0 && !loading) {
      setTimeout(() => {
        ...
        setToast({
          ...
          message:
            'Dataset "' +
            (location.state?.highlightDsName || "") +
            '" da duoc tao thanh cong!',
          severity: "success",
        });
      }, 400);
    }
  ...
  const loadSubtopicPool = async (subtopicIds) => {
    const ids = Array.isArray(subtopicIds)
      ? subtopicIds.filter(Boolean)
      : subtopicIds
        ? [subtopicIds]
        : [];
    if (!ids.length) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterStatus !== "all")
      r = r.filter(
        (ds) => getDsStatus(statusByDs[ds.id]).status === filterStatus,
      );
    r.sort((a, b) => {
      let av, bv;
      if (sortBy === "name") {
      ...
      if (sortBy === "items") {
        av = statusByDs[a.id]?.totalRawItems || 0;
        bv = statusByDs[b.id]?.totalRawItems || 0;
      } else if (sortBy === "approved") {
        av = statusByDs[a.id]?.counts?.approved || 0;
        bv = statusByDs[b.id]?.counts?.approved || 0;
      } else if (sortBy === "progress") {
        const sg = (id) => {
          const s = statusByDs[id];
          const raw = s?.totalRawItems || 0;
          const ap = s?.counts?.approved || 0;
          return raw > 0 ? (ap / raw) * 100 : 0;
        };
        av = sg(a.id);
        bv = sg(b.id);
      } else {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      annotating: vals.filter((v) => getDsStatus(v).status === "annotating")
        .length,
      review: vals.filter((v) => getDsStatus(v).status === "under_review")
        .length,
    };
  }, [statusByDs]);
  ...
  const handleCreate = async () => {
    ...
    if (!selectedSubtopicIds.length)
      return alert("Chon Topic va it nhat 1 Subtopic");
    setCreating(true);
    setError(null);
    try {
      const payload = {
      ...
      setError(
        "Loi tao dataset: " + (err.response?.data?.message || err.message),
      );
    } finally {
      setCreating(false);
    }
  ...
  const handleOpenEdit = async (ds) => {
    ...
    setForm({
      ...
      type: ds.type || "image",
      subtopicId: ds.subtopicId || "",
    });
    setSelectedTopicId("");
    setEditTopicName("");
    const existingSubtopicIds =
      Array.isArray(ds.subtopicIds) && ds.subtopicIds.length > 0
        ? ds.subtopicIds.map((id) => String(id))
        : ds.subtopicId
          ? [String(ds.subtopicId)]
          : [];
    ...
    if (ds.subtopicId) {
      try {
        const subRes = await axios.get(
          `${API_URL}/api/subtopics/${ds.subtopicId}`,
        );
        const topicId = subRes?.data?.topic_id || "";
        if (topicId) {
          setSelectedTopicId(topicId);
          setEditTopicName(subRes?.data?.topicName || "");
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setToast({
        open: true,
        message: "Cap nhat dataset thanh cong",
        severity: "success",
      });
    } catch (err) {
      alert(
        "Loi cap nhat dataset: " + (err.response?.data?.message || err.message),
      );
    } finally {
      setUpdating(false);
    }
  ...
  const handleOpenDetail = async (ds) => {
    ...
    try {
      ...
      setDetailDs((prev) => ({
        ...prev,
        ...dsRes.data,
        statusData: dsRes.data,
      }));

      // Collect rawItems from ALL subtopics under the topic — always fetch all
      let rawItems = dsRes.data?.data_items || dsRes.data?.items || [];
      const dsData = dsRes.data || {};

      // Collect ALL subtopic IDs to fetch assets from
      const allSubtopicIds = [];

      // From subtopicIds array
      if (Array.isArray(dsData.subtopicIds)) {
        allSubtopicIds.push(...dsData.subtopicIds.filter(Boolean));
      }
      // From subtopics array (each item has .id)
      if (Array.isArray(dsData.subtopics)) {
        dsData.subtopics.forEach(s => {
          const id = s.id || s.subtopicId;
          if (id && !allSubtopicIds.includes(id)) allSubtopicIds.push(id);
        });
      }
      // From single subtopicId field
      const singleStId = dsData.subtopicId || dsData.subtopic_id;
      if (singleStId && !allSubtopicIds.includes(singleStId)) {
        allSubtopicIds.push(singleStId);
      }

      // Fetch subtopics from topic_id if no subtopics found yet
      if (allSubtopicIds.length === 0) {
        const topicId = dsData.topic_id || dsData.topicId;
        if (topicId) {
          try {
            const subsRes = await axios.get(`${API_URL}/api/subtopics?topic_id=${topicId}`);
            const subs = getArray(subsRes.data);
            subs.forEach(s => {
              const id = s.id || s.subtopicId;
              if (id && !allSubtopicIds.includes(id)) allSubtopicIds.push(id);
            });
      ...
      // Fetch assets from ALL subtopics
      for (const stId of allSubtopicIds) {
        try {
          const [subRes, assetRes] = await Promise.all([
            axios.get(`${API_URL}/api/subtopics/${stId}`),
            axios.get(`${API_URL}/api/subtopics/${stId}/assets`),
          ]);
          const subName = subRes.data?.name || 'Subtopic';
          const items = Array.isArray(assetRes.data) ? assetRes.data : [];
          if (items.length > 0) {
            const mapped = items.map(it => ({ ...it, subtopicName: subName }));
            rawItems.push(...mapped);
          }
        } catch {}
      }
      setDetailItems(Array.isArray(rawItems) ? rawItems : []);

      // Tinh summary theo subtopic de hien thi filter chips
      const summaryMap = {};
      for (const item of rawItems) {
        const sid = item.subtopic_id || item.subtopicId || "__none__";
        if (!summaryMap[sid]) {
          summaryMap[sid] = {
            totalItems: 0,
            approved: 0,
            ...
            inReview: 0,
            rejected: 0,
            subtopicName: item.subtopicName || "Subtopic",
          };
        }
        summaryMap[sid].totalItems++;
        if (item.status === "approved") summaryMap[sid].approved++;
        else if (item.status === "submitted" || item.status === "in_review")
          summaryMap[sid].inReview++;
        else if (item.status === "rejected") summaryMap[sid].rejected++;
        else summaryMap[sid].pending++;
      }
      setDetailSubtopicSummary(Object.values(summaryMap));
    } catch {
    } finally {
  ...
  const handleOpenExport = async (ds) => {
    ...
    try {
      ...
      const previewItems = (s?.finalItems || []).slice(0, 5).map((item) => {
        ...
        return {
          filename:
            item.dataItem?.originalName || item.dataItem?.filename || "unknown",
          labels: arr
            .filter(Boolean)
            .map((l) => (typeof l === "string" ? l : l.label || l.text || l)),
        };
      });
      const dist = {};
      (s?.finalItems || []).forEach((item) => {
        ...
        lbls.forEach((l) => {
          const k = typeof l === "string" ? l : l.label || l.text || "unknown";
          dist[k] = (dist[k] || 0) + 1;
        });
      });
      setExportPreview({
        ...
        formats: ["JSON", "YOLO", "VOC", "COCO", "CSV"],
      });
    } catch {
    } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      const resp = await axios.get(
        `${API_URL}/api/projects/${exportDs.id}/export`,
        { responseType: "blob" },
      );
      ...
      alert(err.response?.data?.message || "Loi export");
    }
  };
    ...
    >
          ...
          >
            <Box>
              ...
              <Typography variant="body2" sx={{ color: "#94a3b8" }}>
                Quan ly bo du lieu cho AI Training
              </Typography>
            </Box>
            {!isAdmin && (
              <Button
        ...
        <Box sx={{ p: 2, borderBottom: "1px solid #334155" }}>
          ...
          >
            ...
            <Box sx={{ display: "flex", gap: 1, ml: "auto" }}>
              <Chip
                label={`${filtered.length} / ${datasets.length}`}
                size="small"
          ...
          {showFilters && (
            ...
            >
              ...
              <FormControl size="small" sx={{ minWidth: 150 }}>
                ...
                >
                  <MenuItem value="all">Tat ca</MenuItem>
                  <MenuItem value="ready">San sang AI</MenuItem>
                  <MenuItem value="under_review">Cho Review</MenuItem>
                  <MenuItem value="annotating">Dang ghi nhan</MenuItem>
                  <MenuItem value="not_started">Chua bat dau</MenuItem>
                </Select>
              </FormControl>
              <FormControl size="small" sx={{ minWidth: 130 }}>
                ...
                >
                  <MenuItem value="createdAt">Ngay tao</MenuItem>
                  <MenuItem value="name">Ten</MenuItem>
                  <MenuItem value="items">So items</MenuItem>
                  <MenuItem value="approved">Da duyet</MenuItem>
                  <MenuItem value="progress">Tien do</MenuItem>
                </Select>
              </FormControl>
              <Button
                size="small"
                ...
                startIcon={<SortIcon />}
                sx={{ ...btnSecondary, height: 38 }}
              >
                {sortOrder === "desc" ? "Giam dan" : "Tang dan"}
              </Button>
              <Button
                size="small"
                onClick={() => {
          ...
          {viewMode === "card" ? (
            <Grid container spacing={2.5}>
              {filtered.map((ds) => {
                ...
                const raw =
                  stat?.totalRawItems || ds.totalItems || ds.files?.length || 0;
                const approved = stat?.counts?.approved || 0;
                const pct =
                  raw > 0
                    ? Math.min(Math.round((approved / raw) * 100), 100)
                    : 0;
                      ...
                      <CardContent>
                        ...
                        >
                          ...
                          >
                            {ds.type === "audio" ? (
                              <AudioIcon
                                sx={{ fontSize: 20, color: "#f472b6" }}
                              />
                            ) : ds.type === "text" ? (
                              <TextIcon
                                sx={{ fontSize: 20, color: "#34d399" }}
                              />
                            ) : (
                              <ImageIcon
                        ...
                        <Box sx={{ mb: 1.5 }}>
                          ...
                          >
                            Duyet: {approved} | Tu choi:{" "}
                            {stat?.counts?.rejected || 0} | Review:{" "}
                            {(stat?.counts?.submitted || 0) +
                              (stat?.counts?.pendingAnnotation || 0)}
                          </Typography>
                        </Box>
                        <Box
                          sx={{
                        ...
                        >
                          ...
                          <Typography
                            ...
                            sx={{ color: "#64748b" }}
                          >
                            {(stat?.votes?.approveVotes || 0) +
                              (stat?.votes?.rejectVotes || 0)}
                            /{stat?.votes?.totalVotes || 0} votes
                          </Typography>
                        </Box>
                        <Stack direction="row" spacing={1}>
                          ...
                          <Button
                            ...
                            onClick={() => handleOpenExport(ds)}
                            disabled={si.status !== "ready"}
                            sx={{
                              flex: 1,
                              textTransform: "none",
                              fontWeight: 700,
                              borderColor:
                                si.status === "ready" ? "#22c55e" : "#475569",
                              color:
                                si.status === "ready" ? "#22c55e" : "#64748b",
                              fontSize: "0.75rem",
                            }}
                          >
                            Export
      ...
      >
                ...
                <Grid container spacing={3}>
                      ...
                      {detailDs?.statusData && (
                        <Stack spacing={1.5}>
                          {[
                            {
                              label: "Tong Items",
                              value: detailDs.statusData.totalRawItems,
                              color: "#e2e8f0",
                            },
                            {
                              label: "Da ghi nhan",
                              value: detailDs.statusData.counts?.completed || 0,
                              color: "#60a5fa",
                            },
                            {
                              label: "Dang review",
                              value: detailDs.statusData.counts?.submitted || 0,
                              color: "#fbbf24",
                            },
                            {
                              label: "Da duyet",
                              value: detailDs.statusData.counts?.approved || 0,
                              color: "#22c55e",
                            },
                            {
                              label: "Tu choi",
                              value: detailDs.statusData.counts?.rejected || 0,
                              color: "#ef4444",
                            },
                            {
                              label: "Tong Votes",
                              value: detailDs.statusData.votes?.totalVotes || 0,
                              color: "#a78bfa",
                            },
                            {
                              label: "Vote duyet",
                              value:
                                detailDs.statusData.votes?.approveVotes || 0,
                              color: "#22c55e",
                            },
                            {
                              label: "Vote tu choi",
                              value:
                                detailDs.statusData.votes?.rejectVotes || 0,
                              color: "#ef4444",
                            },
                          ].map((item) => (
                            <Box
                  ...
                  <Grid item xs={12} md={6}>
                    ...
                    >
                      ...
                      {detailDs?.statusData?.finalItems &&
                      detailDs.statusData.finalItems.length > 0 ? (
                        <Box
                          sx={{
                            display: "flex",
              ...
              {detailTab === 1 && (
                <Box>
                  <Typography variant="body2" sx={{ color: "#94a3b8", mb: 2 }}>
                    Tat ca nhan da ghi nhan trong dataset
                  </Typography>
                  {detailDs?.statusData?.finalItems &&
                  detailDs.statusData.finalItems.length > 0 ? (
                    <Grid container spacing={1}>
                      {(() => {
                        ...
                        detailDs.statusData.finalItems.forEach((item) => {
                          const lbls =
                            item.labels?.objects || item.labels?.spans || [];
                          lbls.forEach((l) => {
                            const k = l.label || l.text || "unknown";
                            if (!dist[k])
                              dist[k] = {
                    ...
                    <Box sx={{ textAlign: "center", py: 8, color: "#64748b" }}>
                      <Typography>Chua co nhan nao</Typography>
                    </Box>
                  )}
                </Box>
              )}
              {detailTab === 2 && (
                <Box>
                  <Typography variant="body2" sx={{ color: "#94a3b8", mb: 1 }}>
                    Hien thi {detailItems.length} items
                  </Typography>
                    ...
                    >
                      ...
                      >
                        <Chip
                          ...
                          label={`Tat ca (${detailItems.length})`}
                          sx={{
                            bgcolor:
                            ...
                            color:
                              detailSubtopicFilter === "__all__"
                                ? "#93c5fd"
                                : "#cbd5e1",
                            border: `1px solid ${detailSubtopicFilter === "__all__" ? "rgba(59,130,246,0.7)" : "rgba(148,163,184,0.4)"}`,
                            fontWeight: 700,
                          }}
                        />
                        {detailSubtopicSummary.map((s, idx) => {
                          const key =
                            s.subtopicId?.toString?.() ||
                            s.subtopicId ||
                            "__none__";
                          ...
                          return (
                            <Chip
                              ...
                              label={`${s.subtopicName || "Khong ro"} (${s.totalItems})`}
                              sx={{
                                bgcolor: active
                                  ? "rgba(59,130,246,0.25)"
                                  : "rgba(59,130,246,0.12)",
                                color: active ? "#bfdbfe" : "#93c5fd",
                                border: `1px solid ${active ? "rgba(96,165,250,0.9)" : "rgba(59,130,246,0.4)"}`,
                                fontWeight: 700,
                              }}
                            />
                          );
                      ...
                      {detailItems
                        ...
                        .map((item, idx) => {
                          ...
                          const fn =
                            ...
                            "Unknown";
                          const isText =
                            /\.(txt|csv|json|xml)$/i.test(fn) ||
                            item.mimeType?.startsWith("text/");
                          const isAudio =
                            /\.(mp3|wav|ogg|m4a|aac|flac)$/i.test(fn) ||
                            item.mimeType?.startsWith("audio/");
                          const anns = (item.annotations || []).filter(
                            (a) => a.status === "approved",
                          );
                          return (
                            <Grid
                            ...
                            >
                              <Box
                                onClick={() =>
                                  navigate(
                                    `/manager/datasets/${detailDs?.id}/items/${item.id || idx}`,
                                    {
                                      state: {
                                        item,
                                        datasetName: detailDs?.name,
                                      },
                                    },
                                  )
                                }
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setDetailOpen(false)} sx={btnSecondary}>
            Dong
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
            ...
            <Box>
              <Grid container spacing={2} sx={{ mb: 3 }}>
                {[
                  {
                    label: "Tong Items",
                    value: exportPreview.dataset.totalRawItems,
                    color: "#e2e8f0",
                  },
                  {
                    label: "Da Duyet",
                    value: exportPreview.dataset.totalApproved,
                    color: "#22c55e",
                  },
                ].map((item) => (
              ...
              {Object.keys(exportPreview.dataset.labelDistribution).length >
                0 && (
                <Box sx={{ mb: 3 }}>
                  <Typography
                    variant="subtitle2"
                    fontWeight={700}
                    sx={{ mb: 1.5, color: "#e2e8f0" }}
                  >
                    Phan bo nhan
                  </Typography>
                  <Box
                  ...
                  >
                    {Object.entries(exportPreview.dataset.labelDistribution)
                      .sort((a, b) => b[1] - a[1])
                      .slice(0, 10)
                      .map(([k, v], i) => {
                        const colors = [
                        ...
                        return (
                          <ChartBar
                            ...
                            max={
                              Object.values(
                                exportPreview.dataset.labelDistribution,
                              )[0]
                            }
                  ...
                  {exportPreview.formats.map((fmt) => (
                    <Chip
                      ...
                      sx={{
                        fontWeight: 700,
                        cursor: "pointer",
                        bgcolor: exportFormat === fmt ? "#2563eb" : "#334155",
                        color: exportFormat === fmt ? "#fff" : "#94a3b8",
                        border:
                          exportFormat === fmt
                            ? "2px solid #3b82f6"
                            : "1px solid #475569",
                        "&:hover": {
                          bgcolor: exportFormat === fmt ? "#3b82f6" : "#475569",
                        },
                      }}
                    />
                  ))}
              ...
              {exportPreview.dataset.sampleItems.length > 0 && (
                <Box>
                  <Typography
                    ...
                    sx={{ mb: 1.5, color: "#e2e8f0" }}
                  >
                    Xem truoc ({exportPreview.dataset.sampleItems.length} items)
                  </Typography>
                  <Box
                    sx={{
                      maxHeight: 200,
                    ...
                    {exportPreview.dataset.sampleItems.map((item, i) => (
                      <Box
                        ...
                        sx={{
                          py: 1,
                          borderBottom:
                            i < exportPreview.dataset.sampleItems.length - 1
                              ? "1px solid #334155"
                              : "none",
                        }}
                      >
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setExportOpen(false)} sx={btnSecondary}>
            Huy
          </Button>
          <Button
            variant="contained"
            ...
            disabled={
              !exportPreview || exportPreview.dataset.totalApproved === 0
            }
            startIcon={<DownloadIcon />}
            sx={{
              ...btnPrimary,
        ...
        <DialogContent>
          ...
          >
            <Typography
              variant="subtitle2"
              fontWeight={700}
              sx={{ color: "#60a5fa", mb: 1.5 }}
            >
              Dataset Context (Taxonomy Structure)
            </Typography>
                ...
                >
                  {subtopics.map((s) => (
                    <MenuItem key={s.id} value={s.id}>
                      <Checkbox
                        checked={selectedSubtopicIds.indexOf(s.id) > -1}
                      />
            ...
            {selectedSubtopicIds.length > 0 && (
              ...
              >
                Subtopic context is linked. Label sets se duoc ke thua tu
                subtopic dau tien duoc chon.
              </Typography>
            )}
          </Box>
          ...
          {/* Subtopics Info */}
          {selectedSubtopicIds.length > 0 &&
          Object.keys(subtopicData).length > 0 ? (
            <Box
              sx={{
                p: 2,
                border: "1px solid #334155",
                    ...
                    return (
                      ...
                      >
                        ...
                        {subInfo.labels && subInfo.labels.length > 0 && (
                          ...
                          >
                            {subInfo.labels.map((ls, li) => (
                              <Chip
                                key={ls.id ?? ls.name ?? li}
                                label={ls.name}
                                size="small"
                                sx={{
            ...
            >
              <Typography variant="body2" sx={{ color: "#64748b" }}>
                Chon Topic va Subtopic de xem kho du lieu
              </Typography>
            </Box>
          )}
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button onClick={() => setCreateOpen(false)} sx={btnSecondary}>
            Huy
          </Button>
          <Button
            ...
            variant="contained"
            disabled={
              creating || !form.name.trim() || selectedSubtopicIds.length === 0
            }
            sx={btnPrimary}
          >
            {creating ? (
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button onClick={() => setEditOpen(false)} sx={btnSecondary}>
            Huy
          </Button>
          <Button
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Ban co that su muon xoa dataset{" "}
            <strong>"{selectedDs?.name}"</strong>?
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button onClick={() => setDeleteOpen(false)} sx={btnSecondary}>
            Huy
          </Button>
          <Button
      ...
      >
        <Alert
          ...
          sx={{
            bgcolor: toast.severity === "error" ? "#7f1d1d" : "#14532d",
            color: "#fff",
          }}
        >
          {toast.message}
