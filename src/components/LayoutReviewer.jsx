...
const LayoutReviewer = () => {
  ...
  const isActive = (path) => {
    if (path === '/reviewer') return location.pathname === path;
    return location.pathname === path || location.pathname.startsWith(`${path}/`);
  };

  // Hide sidebar on workspace pages
  if (location.pathname.startsWith('/reviewer/workspace')) {
    return (
    ...
    <div className="flex h-screen bg-slate-900 text-gray-200">
      <aside className="fixed top-0 left-0 w-64 h-screen bg-gray-900 border-r border-gray-800 flex flex-col shrink-0 z-50">
        ...
        <div className="border-b border-gray-800 px-5 py-4 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] uppercase tracking-wider text-violet-400 font-semibold">Reviewer</p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-gray-800 p-3 shrink-0 sticky bottom-0 bg-gray-900 z-10">
          ...
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-violet-600 border border-gray-700 shrink-0">
